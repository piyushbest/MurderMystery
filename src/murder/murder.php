...
class murder extends PluginBase implements Listener {
       ...
       public $prefix = TE::GRAY . "[" . TE::AQUA . TE::BOLD . "§cMurder" . TE::RESET . TE::GRAY . "]";
	public $mode = 0;
	public $arenas = array();
	public $vote = true;
	public $swop_voted;
	...
	{
		  $this->getLogger()->info(TE::DARK_AQUA . "Murder by TheGrittlex");
$this->path = $this->getDataFolder();
        ...
        switch($cmd->getName()){
			...
			return true;
                            ...
                            if($player->isOp())
				{
					...
					$player->setNameTag("§f§o§b§b" . $player->getName());
	...
	public function onInteract(PlayerInteractEvent $event)
	{
		...
		if($tile instanceof Sign) 
		{
			if($this->mode==26)
			{
				$tile->setText(TE::AQUA . "[Join]",TE::YELLOW  . "0 / 20","§f§o§b" . $this->currentLevel,$this->prefix);
				$this->refreshArenas();
				$this->currentLevel = "";
				$this->mode = 0;
						...
						$level = $this->getServer()->getLevelByName($namemap);
                                                if($slots->get("slot1".$namemap)==null)
                                                {
                                                        ...
                                                        $slots->set("slot1".$namemap, $player->getName());
                                                         $player->setNameTag("§f§o§b" . $player->getName());
 ...
 $player->getInventory()->setItem(8, Item::get(Item::BLAZE_ROD, 0, 1));
                                                        ...
                                                        $player->setNameTag("§3§f§o§3" . $player->getName());
                                                        foreach($level->getPlayers() as $playersinarena)
                                                        {
	...
	public function onRun($tick)
	{
		...
		if(!empty($arenas))
		{
			foreach($arenas as $arena)
			{
                ...
                $fuerza->setAmplifier(3);
                $fuerza->setVisible(false);
                $fuerza->setDuration(1000000);
                $pl->addEffect($fuerza);
                $item = Item::get(Item::DIAMOND_SWORD, 0, 1);
                $item->addEnchantment(Enchantment::getEnchantment(10)->setLevel(2));
                $pl->setNameTag("§c(Asesino)" . $pl->getName());
                ...
                $pl->getInventory()->setItem(1, $item);
                $pl->setNameTag("§f§o§b" . $pl->getName());
                $pl->getInventory()->setItem(0, Item::get(Item::BREAD, 0, 1));
                $pl->sendTitle("§bDetective");
                 $fuerza = Effect::getEffect(5);
                $fuerza->setAmplifier(3);
                $fuerza->setVisible(false);
                $fuerza->setDuration(1000000);
             ...
             $pl->setNameTag("§f§o§b." . $pl->getName());
                ...
                break;
							...
							else
							{
                                                                ...
                                                                $ase = substr_count($names, "§c(Asesino)");
                                                                $ino = substr_count($names, "§f§o§b");
                                                                foreach($playersArena as $pla)
                                                                {
                                                                    if(strpos($pla->getNameTag(), "§c(Asesino)") !== false)
                                                                    {
                                                                ...
                                                                foreach($playersArena as $pla)
                                                                {
                                                                   ...
                                                                   $pla->sendPopup("§o§b $player: Inocente");
                                                                        $pla->sendTip(TE::RED."Murder:" . $ase .TE::AQUA. " Inocentes:" . $ino);
                                                                ...
                                                                if($ase!=0 && $ino==0)
                                                                {
                                                                    ...
                                                                    foreach($playersArena as $pl)
                                                                    {
                                                                        ...
                                                                        if(strpos($pl->getNameTag(), "§c(Asesino)") !== false)
                                                                        {
